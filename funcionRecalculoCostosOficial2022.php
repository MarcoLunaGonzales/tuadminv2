	...
	if($codigoItem!=0 && $codigoItem!=""){
		...
		while($datMateriales=mysql_fetch_array($respMateriales)){
			...
			{	$fecha_consulta=$vector_final_fechas[$indice];
				...
				i.ingreso_anulado=0 and id.cod_material='$codigoItem' and i.fecha='$fecha_consulta' order by i.nro_correlativo";
				...
				{	$nro_ingreso=$dat_ingresos[0];
					...
					if ( $codTipoIngreso==1000 || $codTipoIngreso==1001 ){
						//INGRESO NORMAL DESDE DISTRIBUIDOR MANDA EL PRECIO DE COMPRA
			...
			$sqlDel="DELETE from costo_promedio_mes where cod_material='$codigoItem' and mes='12' and anio='2022' and 
					cod_almacen='$rpt_almacen'";
			$respDel=mysql_query($sqlDel);
			$valorInventario=$suma_saldo_final+$nuevoCostoPromedio;			
			$sqlInsert="insert into costo_promedio_mes values('$rpt_almacen', '2022', '12', '$codigoItem', '$suma_saldo_final', '$nuevoCostoPromedio', '$valorInventario')";
