...
<?php
    ...
    // Media Hoja Oficio
    $pdf = new FPDF($orientation='L', $unit='mm', 'Legal');
    $pdf->AddPage();
    ...
    for ($col = 1; $col <= 3; $col++) {
        ...
        $pdf->Cell(35, $textypos, $cab_dir_cliente, 'TBR', 0, 'L');
        // $pdf->MultiCell(27.5, 1, utf8_decode($cab_dir_cliente), 0, 'L');
        $pdf->setY(35);$pdf->setX($ejeX + 76);
        $pdf->Cell(35, $textypos, $cab_tipo_venta, 'TBR', 0, 'L');
        $pdf->Ln();
        ...
        while($dataSalidaDet = mysqli_fetch_array($respSalidaDet)){
            ...
            $start_y = $pdf->GetY();
            $pdf->multiCell(70.5, 3.5 * $row_index, utf8_decode($nombreProductoX), 1, 'LTRB', false);
            $end_y = $pdf->GetY();
        ...
        $pdf->SetFont('Arial','B',6);  
        $pdf->setX($ejeX + 3);
        $pdf->MultiCell(70.5, 3, utf8_decode("Son:".$txtMonto." ".$montoDecimal."/100 Bolivianos"), 1, 'L');
