...
echo "<script language='JavaScript'>
		...
		{	var rpt_territorio, rpt_almacen, tipo_item, rpt_linea;
			rpt_territorio=f.rpt_territorio.value;
			rpt_almacen=f.rpt_almacen.value;
			tipo_item=f.tipo_item.value;
			rpt_linea=f.rpt_linea.value;
			window.open('rpt_inv_existenciastransito.php?rpt_territorio='+rpt_territorio+'&rpt_almacen='+rpt_almacen+'&rpt_linea='+rpt_linea+'&tipo_item='+tipo_item+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');
			return(true);
		}
		...
		{	var rpt_territorio, rpt_almacen, tipo_item, rpt_linea;
			...
			tipo_item=f.tipo_item.value;
			rpt_linea=f.rpt_linea.value;
			window.open('rpt_inv_existenciastransito_xls.php?rpt_territorio='+rpt_territorio+'&rpt_almacen='+rpt_almacen+'&rpt_linea='+rpt_linea+'&tipo_item='+tipo_item+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');
			return(true);
		}
...
echo"<form method='post' action=''>";
	...
	echo "<tr><th align='left'>L�nea</th><td><select name='rpt_linea' class='texto'>";
	$sql="select codigo_linea, nombre_linea from lineas where linea_inventarios=1 order by nombre_linea";
	$resp=mysqli_query($enlaceCon,$sql);
	echo "<option value='0' selected>Todas las l�neas</option>";
	while($dat=mysqli_fetch_array($resp))
	{	$codigo_linea=$dat[0];
		$nombre_linea=$dat[1];
