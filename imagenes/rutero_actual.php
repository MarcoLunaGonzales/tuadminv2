...
$resp_borrar_det=mysqli_query($enlaceCon,$sql_borrar_det);*/
...
$resp_gestion=mysqli_query($enlaceCon,$sql_gestion);
$datos_gestion=mysqli_fetch_array($resp_gestion);
...
while($dat_cab=mysqli_fetch_array($resp_cab))
{	$global_visitador=$dat_cab[0];
	...
	{	$codigo_rutero=$datos[0];
		...
		$resp_cod=mysqli_query($enlaceCon,$sql_cod);
		$num_filas=mysqli_num_rows($resp_cod);
		if($num_filas==0)
		{	$cod_contacto=1000;
		}
		else
		{	$dat_cod=mysqli_fetch_array($resp_cod);
		...
		$resp_inserta_extra=mysqli_query($enlaceCon,$sql_inserta_extra);
		...
		while($datos_detalle=mysqli_fetch_array($resp_detalle))
		{	$orden_visita=$datos_detalle[1];
			...
			$resp_inserta_det=mysqli_query($enlaceCon,$sql_inserta_det);
			...
			$resp_inserta_det_extra=mysqli_query($enlaceCon,$sql_inserta_det_extra);	
		}	
	}	
}
