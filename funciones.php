...
function precioVentaCliente($codigo_cliente, $codigo_producto){
	require("conexionmysqlipdf.inc");
	$consulta="select p.precio_cliente from clientes_precios p where p.cod_producto='$codigo_producto' and p.cod_cliente='$codigo_cliente'";
	//echo $consulta;
	$rs=mysqli_query($enlaceCon,$consulta);
	$precioVenta=0;
	while($datVenta=mysqli_fetch_array($rs)){   
		$precioVenta=$datVenta[0];
	}
	$precioVenta=redondear2($precioVenta);
	return $precioVenta;
}
