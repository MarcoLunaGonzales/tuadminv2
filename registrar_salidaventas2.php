...
function setMateriales(f, cod, nombreMat){
	...
	console.log('#########################')
	console.log('materiales'+numRegistro)
	console.log(cod)
	console.log('#########################')
	console.log(document.getElementById('materiales1'))
...
<!-- SELECCIÓN DE ITEMS -->
<?php
	$codigo_venta_duplicado = empty($_GET['cod_venta']) ? 0 : $_GET['cod_venta'];

	if($codigo_venta_duplicado!=0){
		$sql = "SELECT ma.codigo_material, CONCAT(ma.descripcion_material, '(',ma.codigo_material,')') as nombre_material
		FROM salida_detalle_almacenes sda
		LEFT JOIN material_apoyo ma ON ma.codigo_material = sda.cod_material
		WHERE sda.cod_salida_almacen = '$codigo_venta_duplicado'";
		$resp=mysql_query($sql);
		...
		{
			$dup_codigo 		 = $dat[0];
			$dup_nombre_material = $dat[1];
			$index++;
			echo "
			<script language='Javascript'>
				$(document).ready(function(){
					var form1 = {materialActivo: {value:$index}};
					mas(0);
					setMateriales(form1, '$dup_codigo', '$dup_nombre_material');
				});
			</script>";
		}
	}
?>
	...
	<tr bgcolor="#FFFFFF">
		<td width="5%" align="center">
			<a href="javascript:buscarMaterial(form1, 1)"><img src="imagenes/buscar2.png" title="Buscar Producto" width="30"></a>
		</td>
		<td width="30%" align="center">
			<input type="hidden" name="materiales1" id="materiales1" value="0">
			<div id="cod_material1" class="textomedianonegro">-</div>
		</td>
		<td width="10%" align="center">
			<div id="idstock1">
				<input type="hidden" id="stock1" name="stock1" value="">
			</div>
		</td>
		<td align="center" width="8%">
			<input class="inputnumber" type="number" min="1" id="cantidad_unitaria1" onkeyup="calculaMontoMaterial(1);" name="cantidad_unitaria1" onchange="calculaMontoMaterial(1);" step="1" value="1" required=""> 
		</td>
		<td align="center" width="12%">
			<label title="Salvar Precios de Producto">
			<input class="mycheck" type="checkbox" value="" id="salvar_precio1" name="salvar_precio1">
			</label>
			<div id="idprecio1">
				<input class="inputnumber" type="number" min="1" value="0" id="precio_unitario1" name="precio_unitario1" onkeyup="calculaMontoMaterial(1);" onchange="calculaMontoMaterial(1);" step="0.01" required="">
			</div>
		</td>
		<td align="center" width="15%">
			<input class="inputnumber" type="number" value="0" id="descuentoProducto1" name="descuentoProducto1" onkeyup="calculaMontoMaterial(1);" onchange="calculaMontoMaterial(1);" step="0.01">
		</td>
		<td align="center" width="10%">
			<input class="inputnumber" type="number" value="0" id="montoMaterial1" name="montoMaterial1" step="0.01" required="" readonly=""> 
		</td>
		<td align="center" width="10%"><input class="boton2peque" type="button" value="-" onclick="menos(1)"></td>
	</tr>
