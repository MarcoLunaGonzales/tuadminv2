...
$sqlDet="SELECT cod_ordencompra, monto_detalle from pagos_proveedor_detalle where cod_pago='$codPago' ";
$respDet=mysqli_query($enlaceCon,$sqlDet);

while($datDet=mysqli_fetch_array($respDet)){
	$codOrdenCompra = $datDet[0];
	$montoDet	= $datDet[1];

	$sqlUpd="UPDATE ordenes_compra set monto_cancelado = ROUND(monto_cancelado - $montoDet, 2) where codigo=$codOrdenCompra ";
	//echo $sqlUpd;
