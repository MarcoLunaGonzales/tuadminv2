...
function recalculaCostos($codigoItem, $rpt_almacen){
	...
	if($codigoItem!=0 && $codigoItem!=""){
		...
		while($datMateriales=mysql_fetch_array($respMateriales)){
			...
			$sqlCostoAnterior="select c.existencias, c.`costo_unitario` from `costo_promedio_mes` c where 
				c.`cod_almacen`='$rpt_almacen' and c.mes='$mesCosto' and c.`anio`='$anioCosto' and c.`cod_material`='$codigoItem'";
			$respCostoAnterior=mysql_query($sqlCostoAnterior);
			$costoUnitarioAnteriorItem=0;
			$existenciasAnteriorItem=0;
			while($datCostoAnterior=mysql_fetch_array($respCostoAnterior)){
				$existenciasAnteriorItem=$datCostoAnterior[0];
				$costoUnitarioAnteriorItem=$datCostoAnterior[1];
			}
			
			$valorAnteriorItem=$costoUnitarioAnteriorItem*$existenciasAnteriorItem;			
			$nuevoCostoPromedio=$costoUnitarioAnteriorItem;
			
			//Hasta aqui sacamos el costo promedio del 2022
			
			//echo "AntCostoPromedio: ".$nuevoCostoPromedio." existenciasAnteriorItem: ".$existenciasAnteriorItem."<br>";
			//echo "NUEVO VALOR: ".$valorAnteriorItem."<br>";
			...
			$cantidad_kardex=$existenciasAnteriorItem;
